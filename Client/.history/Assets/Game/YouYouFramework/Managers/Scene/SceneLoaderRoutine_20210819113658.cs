...
#else
			GameEntry.Resource.ResourceLoaderManager.LoadMainAsset(AssetCategory.Scenes, GameEntry.Resource.GetSceneAssetBundlePath(sceneName), onComplete: (AssetBundle ab) =>
			{
				//加载场景的资源包
				m_CurrAsync = SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Additive);
				m_CurrAsync.allowSceneActivation = false;
